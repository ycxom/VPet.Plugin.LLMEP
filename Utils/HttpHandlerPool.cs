...
namespace VPet.Plugin.LLMEP.Utils
{
    ...
    internal static class HttpHandlerPool
    {
        ...
        private static readonly ConcurrentDictionary<string, Entry> _entries = new ConcurrentDictionary<string, Entry>(StringComparer.Ordinal);
        private static readonly ConcurrentQueue<(HttpClientHandler Handler, DateTime RetiredUtc)> _retired = new ConcurrentQueue<(HttpClientHandler, DateTime)>();
        ...
        public static HttpClient CreateClient(Func<HttpClientHandler> handlerFactory, TimeSpan? timeout = null)
        {
            var client = new HttpClient(GetHandler(handlerFactory), disposeHandler: false);
            if (timeout.HasValue)
            {
        ...
        private static void DrainRetired(DateTime now)
        {
            while (_retired.TryPeek(out var item) && now - item.RetiredUtc > DisposeGrace)
            {
                ...
                {
                    Logger.Error("HttpHandlerPool", $"释放过期 handler 失败: {ex.Message}");
                }
            }
        }
    }
